    ...
    def create_prompt_template(self):
        if self.role == "MultidisciplinaryTeam":
            templates = f"""
                ...
                Task: Review the patient's medical report from the Cardiologist, Psychologist, and Pulmonologist, analyze them and come up with a list of 3 possible health issues of the patient. If there are less than 3, list all that are present.
                ...
                Psychologist Report: {self.extra_info.get('psychologist_report', '')} # type: ignore # type: ignore
                Pulmonologist Report: {self.extra_info.get('pulmonologist_report', '')} # type: ignore
            """
        else:
            templates = {
                "Cardiologist": """
                    Act like a cardiologist. You will receive a medical report of a patient.
                    Task: Review the patient's cardiac workup, including ECG, blood tests, Holter monitor results, and echo-cardiogram.
            ...
            templates = templates[self.role] # type: ignore
        return PromptTemplate.from_template(templates)
    
    def run(self):
        print(f"{self.role} is running...")
        prompt = self.prompt_template.format(medical_report=self.medical_report) if self.medical_report else self.prompt_template.format()
        try:
